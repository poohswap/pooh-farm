...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.chef = await MasterChef.new(this.poo.address, dev, '1000', '100', { from: minter });
        await this.poo.transferOwnership(this.chef.address, { from: minter });
        await this.poo.setExcludedFromAntiWhale(this.chef.address, true, { from: minter });
    ...
    it('real case', async () => {
      ...
      assert.equal((await this.poo.balanceOf(alice)).toString(), '0');
      await this.chef.deposit(1, '20', { from: alice });
      await this.chef.withdraw(1, '20', { from: alice });
      assert.equal((await this.poo.balanceOf(alice)).toString(), '263');

      await this.poo.approve(this.chef.address, '1000', { from: alice });
      await this.chef.enterStaking('20', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.poo.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async () => {
      ...
      assert.equal((await this.poo.balanceOf(alice)).toString(), '999');
      assert.equal((await this.poo.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.poo.balanceOf(bob)).toString(), '125');
    ...
    it('updaate multiplier', async () => {
      ...
      await this.poo.approve(this.chef.address, '100', { from: alice });
      await this.poo.approve(this.chef.address, '100', { from: bob });
      ...
      assert.equal((await this.poo.balanceOf(alice)).toString(), '700');
      assert.equal((await this.poo.balanceOf(bob)).toString(), '150');
